  ...
  const vfsPlugin: Plugin = {
    ...
    setup(build: PluginBuild) {
      ...
      build.onLoad({ filter: /.*/, namespace: 'external' }, async (args: OnLoadArgs): Promise<OnLoadResult | undefined> => {
        ...
        if (args.path === 'react-dom/server') {
          return {
            contents: `module.exports = require('react-dom/server');`,
            loader: 'js',
          };
        }
        if (args.path === 'deadsimpleseo-react') {
          // Return code that will call require() at runtime in the VM
          // This ensures it uses the VM's React instance
          return {
            contents: `
// Export everything from the runtime-loaded module
// This will be resolved by the VM's require function
const dsrModule = require('deadsimpleseo-react');
module.exports = dsrModule;
`,
            loader: 'js',
          };
        }
  ...
  const require = (moduleName: string) => {
    ...
    if (moduleName === 'deadsimpleseo-react') {
      // Use Node's require to get the actual installed package
      // This will resolve from the project's node_modules
      return nodeRequire('deadsimpleseo-react');
    }
    console.log('Unhandled require in VM for module:', moduleName);
  };

  // Pre-load deadsimpleseo-react into a module cache
  const deadsimpleSEOReactModule = nodeRequire('deadsimpleseo-react');
  const requireCache: Record<string, any> = {
    'deadsimpleseo-react': deadsimpleSEOReactModule,
  };

  const requireWithCache = (moduleName: string) => {
  ...
  const sandbox: any = {
    ...
    setOutput,
    globalThis: {
      __deadsimpleSEOReactModule: deadsimpleSEOReactModule,
    },
    util: {
      deprecate: () => {},
    },
