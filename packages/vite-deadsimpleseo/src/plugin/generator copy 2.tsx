    ...
    setup(build: PluginBuild) {
      ...
      build.onLoad({ filter: /.*/, namespace: 'external' }, async (args: OnLoadArgs): Promise<OnLoadResult | undefined> => {
        ...
        if (args.path === 'deadsimpleseo-react') {
          ...
          return {
            contents: `
// Use a different variable name to avoid esbuild's circular reference detection
const __dsrModule = (function() {
  return require('deadsimpleseo-react');
})();
module.exports = __dsrModule;
`,
            loader: 'js',
          };
        }
  ...
  const require = (moduleName: string) => {
    ...
    if (moduleName === 'deadsimpleseo-react') {
      ...
      // BUT it will use the React instance from the sandbox because 
      // deadsimpleseo-react's CJS build does require('react')
